...
void makeplot(const char *name, TTree *tree, TCut weight, const char *drawstring, const char *xlabel, int nbins, double xlow, double xhigh, int typeMC, const char *qcut) {
  ...
  TH1D *hall = new TH1D(TString::Format("hall_%s",name),TString::Format("%s GeV",qcut),nbins,xlow,xhigh);
  TH1D *hmult0 = new TH1D(TString::Format("hmult0_%s",name),TString::Format("%s GeV",qcut),nbins,xlow,xhigh);
  TH1D *hmult1 = new TH1D(TString::Format("hmult1_%s",name),TString::Format("%s GeV",qcut),nbins,xlow,xhigh);
  TH1D *hmult2 = new TH1D(TString::Format("hmult2_%s",name),TString::Format("%s GeV",qcut),nbins,xlow,xhigh);
  TH1D *hmult3 = new TH1D(TString::Format("hmult3_%s",name),TString::Format("%s GeV",qcut),nbins,xlow,xhigh);
  TH1D *hmult4 = new TH1D(TString::Format("hmult4_%s",name),TString::Format("%s GeV",qcut),nbins,xlow,xhigh);
...
void plotdjr(const TString & infile, const TString & outfile, const char *qcut) {
  ...
  gStyle->SetOptStat(0);
  TCanvas *djr0 = new TCanvas("djr0", "djr0", 800, 600);
  djr0->cd();
  makeplot("djr0",tree,weight,"log10(GenEvent.DJRValues_[0])","DJR 0->1",nbins,djrmin,djrmax,typeMC,qcut);
  TCanvas *djr1 = new TCanvas("djr1", "djr1", 800, 600);
  djr1->cd();
  makeplot("djr1",tree,weight,"log10(GenEvent.DJRValues_[1])","DJR 1->2",nbins,djrmin,djrmax,typeMC,qcut);
  TCanvas *djr2 = new TCanvas("djr2", "djr2", 800, 600);
  djr2->cd();
  makeplot("djr2",tree,weight,"log10(GenEvent.DJRValues_[2])","DJR 2->3",nbins,djrmin,djrmax,typeMC,qcut);
  TCanvas *djr3 = new TCanvas("djr3", "djr3", 800, 600);
  djr3->cd();
  makeplot("djr3",tree,weight,"log10(GenEvent.DJRValues_[3])","DJR 3->4",nbins,djrmin,djrmax,typeMC,qcut);
  TCanvas *djr4 = new TCanvas("djr4", "djr4", 800, 600);
  djr4->cd();
  makeplot("djr4",tree,weight,"log10(GenEvent.DJRValues_[4])","DJR 4->5",nbins,djrmin,djrmax,typeMC,qcut);
